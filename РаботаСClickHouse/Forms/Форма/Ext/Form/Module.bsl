...
Процедура ВыгружаемыйОбъектМетаданныхПриИзменении(Элемент)	
	...
	УстановитьПолноеИмяОМД(ПолноеИмяОМД, ВыгружаемыйТипОМД, ВыгружаемыйОМД);
КонецПроцедуры
...
&НаСервереБезКонтекста
Процедура УстановитьПолноеИмяОМД(ПолноеИмяОМД, ВыгружаемыйТипОМД, ВыгружаемыйОМД)
	
	ПолноеИмяОМД = Метаданные[ВыгружаемыйТипОМД][ВыгружаемыйОМД].ПолноеИмя();
...
Процедура ВыполнитьЗапрос(Команда)
	...
	Запрос = Новый HTTPЗапрос("");                               
	Запрос.УстановитьТелоИзСтроки(ТекстЗапроса.ПолучитьТекст() + " " + ФорматОтвета);
	...
	Если Ответ.КодСостояния = 200 И ФорматОтвета = "FORMAT JSON" Тогда
...
&НаКлиенте
Процедура ГруппаСтраницыНастройкаПриСменеСтраницы(Элемент, ТекущаяСтраница)
	
	Если НЕ Элементы.ГруппаСтраницыНастройка.ТекущаяСтраница = Элементы.НастройкаФормированияДанных Тогда
		Возврат;
	КонецЕсли;
	
	Если ПустаяСтрока(ВыгружаемыйТипОМД) ИЛИ ПустаяСтрока(ВыгружаемыйОМД) Тогда
		Возврат;
	КонецЕсли;
	...
	ТекстЗапроса1С_ = "Выбрать Строки Из ТипОМД.ОМД КАК ОМД";
	
	Строки = "";
	
	Для Каждого СтрокаРеквизит Из НайденныеСтроки Цикл
		Строки = Строки + ВыгружаемыйОМД + "." + СтрокаРеквизит.ИмяРеквизита + " КАК " + СтрокаРеквизит.ИмяРеквизита + ",";
	КонецЦикла;
	
	Строки = Лев(Строки, СтрДлина(Строки) - 1);
	
	ТекстЗапроса1С_ = СтрЗаменить(ТекстЗапроса1С_, "Строки", Строки);
	ТекстЗапроса1С_ = СтрЗаменить(ТекстЗапроса1С_, "ТипОМД.ОМД", ПолноеИмяОМД);
	ТекстЗапроса1С_ = СтрЗаменить(ТекстЗапроса1С_, "КАК ОМД", "КАК " + ВыгружаемыйОМД);
	
	ТекстЗапроса1С.УстановитьТекст(ТекстЗапроса1С_);
...
Процедура КонструкторЗапроса(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьКонструкторЗапросов", ЭтотОбъект);
	
	Конструктор = Новый КонструкторЗапроса(ТекстЗапроса1С.ПолучитьТекст());
	Конструктор.Показать(ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработатьКонструкторЗапросов(Текст, ДопПараметры) Экспорт 
	
	Если Текст = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ТекстЗапроса1С.УстановитьТекст(Текст);
	
	
КонецПроцедуры

&НаКлиенте
Процедура СформироватьКомандыВыгрузки(Команда)
	
	РезультатРаботыЗапроса = ВыполнитьЗапрос1СНаСервере();
...
Функция ВыполнитьЗапрос1СНаСервере()
	...
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапроса1С.ПолучитьТекст();
	
	Для Каждого СтрокаПараметр Из ПараметрыЗапроса1С Цикл		
		Запрос.УстановитьПараметр(СтрокаПараметр.Наименование, СтрокаПараметр.Значение);		
	КонецЦикла;
